...
def test(args, split, loader, model, log, epoch, recorder):
    ...
    with torch.no_grad():
        for i, sample in enumerate(loader):
            ...
            print(data['tar'].data.shape)
            print(out_var.data.shape)
            print(data['m'].data.shape)
            acc, angular_error_map = eval_utils.calNormalAcc(data['tar'].data, out_var.data, data['m'].data)
            ...
            if iters % save_intv == 0:
                ...
                log.saveNormalResults(angular_error_map['angular_map'], split, epoch+10, iters)
